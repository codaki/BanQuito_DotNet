...
namespace CLICON_EUREKABANK.Vista
{
    public class GenerarTablaView
    {
        ...
        public async Task Run()
        {
            ...
            {
                // Fetch the client ID
                var responseCliente = await _controller.ObtenerCodigoCliente(_cedulaCliente);
                ...
                string result = await _controller.CrearTablaAmortizacionService(idCliente.Value, _totalCliente, numCuotas);
                var resultado = await _controller.CrearFacturaService(idCliente.Value, _totalCliente, "CR", _cantidad);
