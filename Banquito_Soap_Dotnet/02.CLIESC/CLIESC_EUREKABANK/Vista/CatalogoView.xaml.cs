...
namespace CLIESC_EUREKABANK.Vista
{
    ...
    public partial class CatalogoView : Window
    {
        ...
        private async void AgregarButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                string result = await _controller.AgregarTelefonoService(nombre, precio, _base64Image);

                MensajeLabel.Content = result == "Teléfono insertado exitosamente."
        ...
        private async void EliminarButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                string result = await _controller.EliminarTelefonoService(id);

                // Update the label with success or error message
                MensajeLabel.Content = result == "Teléfono eliminado exitosamente."
        ...
        private async void ActualizarButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                string result = await _controller.ActualizarTelefonoService(codTel, nombre, precio,_base64Image);

                // Update the label with success or error message
                MensajeLabel.Content = result == "Teléfono actualizado exitosamente."
